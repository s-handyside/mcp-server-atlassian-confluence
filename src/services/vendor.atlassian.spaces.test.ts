...
describe('Vendor Atlassian Spaces Service', () => {
	...
	describe('get', () => {
		...
		it('should include expanded fields when requested', async () => {
			...
			// Check for requested expanded fields
			expect(result).toHaveProperty('labels');
			expect(result.labels?.results).toBeDefined();
			expect(result).toHaveProperty('description');
			expect(result.description?.view).toBeDefined();
			expect(result).toHaveProperty('icon');
			// Icon might be null even if requested, so just check property exists
		}, 30000);

		it('should throw an McpError for an invalid space ID format', async () => {
			if (skipIfNoCredentials()) return;

			const invalidIdFormat = 'INVALID-SPACE-ID';

			await expect(
				atlassianSpacesService.get(invalidIdFormat),
			).rejects.toThrow(McpError);

			try {
				await atlassianSpacesService.get(invalidIdFormat);
			} catch (e) {
				expect(e).toBeInstanceOf(McpError);
				// Confluence API might return 400 or 404 depending on format validity vs existence
				expect([400, 404]).toContain((e as McpError).statusCode);
			}
		}, 30000);

		it('should throw an McpError for a non-existent space ID', async () => {
			if (skipIfNoCredentials()) return;

			const nonExistentId = '999999999'; // Assume this numeric ID doesn't exist
