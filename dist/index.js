...
Object.defineProperty(exports, 'logger', {
	enumerable: true,
	get: function () {
		return logger_util_js_1.logger;
	},
});
...
Object.defineProperty(exports, 'config', {
	enumerable: true,
	get: function () {
		return config_util_js_1.config;
	},
});
const error_util_js_1 = require('./utils/error.util.js');
const index_js_1 = require('./cli/index.js');
const ipaddress_tool_js_1 = __importDefault(
	require('./tools/ipaddress.tool.js'),
);
const ipaddress_resource_js_1 = __importDefault(
	require('./resources/ipaddress.resource.js'),
);
...
async function startServer(mode = 'stdio') {
	...
	logger_util_js_1.logger.info(
		`[src/index.ts] DEBUG value: ${process.env.DEBUG}`,
	);
	logger_util_js_1.logger.info(
		`[src/index.ts] IPAPI_API_TOKEN value exists: ${Boolean(process.env.IPAPI_API_TOKEN)}`,
	);
	logger_util_js_1.logger.info(
		`[src/index.ts] Config DEBUG value: ${config_util_js_1.config.get('DEBUG')}`,
	);
	...
	if (mode === 'stdio') {
		...
		throw (0, error_util_js_1.createUnexpectedError)(
	...
	return serverInstance.connect(transportInstance).catch((err) => {
		logger_util_js_1.logger.error(
			`[src/index.ts] Failed to start server`,
			err,
		);
		process.exit(1);
	});
